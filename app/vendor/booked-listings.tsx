  ...
  const fetchListings = async (forceRefresh = false) => {
    try {
      ...
      const response = await fetchWithCache<{ content: Listing[] }>(
        `${BASE_URL}/vendors/booked-listings?page=0&size=20&sort=${springSort}`,
        {
          headers: {
    ...
    useCallback(() => {
      const smartRefresh = async () => {
        ...
        if (Date.now() - lastFetchTime.current > CACHE_DURATION) {
          console.log('Data stale (>15min), refreshing booked listings...');
          await fetchListings(true);
          return;
        }
        
        // Case 3: Data fresh (within 15min) - use cache, no API call
        console.log('Booked listings data is fresh (within 15min), using cache');
      };
