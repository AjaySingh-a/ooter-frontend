...
export default function BookedListingsScreen() {
  ...
  const fetchListings = async (forceRefresh = false) => {
    try {
      ...
      const cacheKey = `booked-listings-${userId}-${springSort}`;

      const response = await fetchWithCache<{ content: Listing[] }>(
        `${BASE_URL}/vendors/booked-listings?page=0&size=20&sort=${springSort}&t=${Date.now()}`,
        {
          headers: {
      ...
      console.log('Fetched fresh booked listings:', response?.content);
    } catch (e) {
      console.error('❌ API Error:', e);
      Alert.alert('Error', 'Failed to fetch booked listings');
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const clearCache = async () => {
    ...
    const userId = JSON.parse(atob(token.split('.')[1]))?.userId || 'guest';
    const cacheKeys = [
      `booked-listings-${userId}-name,asc`,
      `booked-listings-${userId}-name,desc`,
      `booked-listings-${userId}-pricePerMonth,asc`,
      `booked-listings-${userId}-pricePerMonth,desc`
    ];
    ...
    useCallback(() => {
      const smartRefresh = async () => {
        ...
        if (!data.length || !lastFetchTime.current) {
          console.log('First load, fetching booked listings...');
        ...
        if (Date.now() - lastFetchTime.current > CACHE_DURATION) {
          console.log('Data stale (>5min), refreshing booked listings...');
          await fetchListings(true);
          return;
        }
        
        // Case 3: Data fresh (within 5min) - use cache, no API call
        console.log('Booked listings data is fresh (within 5min), using cache');
      };
    ...
    <View key={item.id} style={styles.card}>
      ...
      <View style={styles.bottomRow}>
        <View style={styles.column}>
          ...
          <View style={[styles.statusPill, styles.red]}>
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      <Text style={styles.activeTitle}>Booked ({data.length})</Text>
